...
with st.container():
    ...
    # --- تطوير منطق الرد "ابن المنطقة" لكل شي بالحياة ---
    input_lower = user_input.lower()
    
    # 1. السلام والترحيب
    if any(word in input_lower for word in ["هلو", "سلام", "شلونك", "يا هلا"]):
        response = random.choice([
            "يا مية هلا بالطيب، نورت مكتبة أخوك أيوب.. إحنا بخدمتك!",
            "هلا بيك يا غالي، شلون الصحة؟ عساك بخير ونعمة.",
            "يا هلا وكل الهلا، تشرفنا بيك وبجيتك الغالية علينا."
        ])

    # 2. السوالف عن الجو والكهرباء (يومياتنا)
    elif any(word in input_lower for word in ["حر", "صيف", "كهرباء", "طفة", "باردة"]):
        response = "والله يا خوي الجو صاير ميزان حرارة، والكهرباء مثل ما تعرف 'يوم لك ويوم عليك'.. بس إحنا بمكتبة أيوب مجهزين أمورنا، والشغل ما يتوقف لو تطفة الدنيا كلها!"

    # 3. الرياضة والطشة (ريال وبرشا)
    elif any(word in input_lower for word in ["طوبة", "ريال", "برشا", "مباراة", "لعبة"]):
        response = "ها بدأت سوالف الطوبة؟ تدري بينا أهل القصر الأوسط نحب الونسة.. المهم الفريق اللي تشجعه يفوز، بس الأهم تخلص شغلك بمكتبتنا وتطلع راضي وممنون!"

    # 4. الشغل المستعجل والوقت
    elif any(word in input_lower for word in ["مستعجل", "بسرعة", "شوكت", "اليوم"]):
        response = "على راسي، إحنا شعارنا السرعة والدقة. المونتاج والطباعة عندنا مثل البرق، بس املأ الاستمارة حتى نثبت دورك ونخلصك بأقرب وقت."

    # 5. الأكل والعزيمة (كلام عراقي)
    elif any(word in input_lower for word in ["جوعان", "أكل", "عزيمة", "تمن"]):
        response = "أبشر بالعزيمة! بس تخلص معاملتك وتستلم شغلك، نتشرف بيك ونشرب أحلى استكان شاي مهيل.. إحنا أهل كرم وأيوب ما يقصر ويا ضيوفه."

    # 6. الأسعار والخصم (المكاسر)
    elif any(word in input_lower for word in ["غالي", "رخيص", "خصم", "تنزيلات"]):
        response = "لا تشيل هم الفلوس، إحنا جايين نخدمك ونبني صيت طيب. السعر راح يرضيك وزيادة، وإذا عندك شغل هواية، أيوب يسويلك 'سعر خاص' يبيض الوجه."

    # 7. عن أيوب هاني نفسه
    elif any(word in input_lower for word in ["أيوب", "صاحب المحل", "أبو"]):
        response = "أيوب أخوك وصديقك، شاب طموح ومختص بالمونتاج والذكاء الاصطناعي.. هدفه يسهل أمور الناس ويوفرلهم أحلى خدمة بالقصر الأوسط."

    # 8. الوداع
    elif any(word in input_lower for word in ["باي", "مع السلامة", "في مال الله"]):
        response = "في مال الله وحفظه يا طيب، نورتنا بمرورك.. ننتظرك ترجع لنا مرة ثانية. مع السلامة!"

    # 9. رد عام لأي شي ثاني
    else:
        response = "والله كلامك ذهب، بس ركز وياي.. إحنا هنا بمكتبة أيوب غرضنا نسهل شغلك. تحب نسويلك مونتاج لو طباعة لو معاملات زين كاش؟"
